...
function wireUI() {
  ...
  const onboardingOpenBtn = document.getElementById('onboarding-open-settings-btn');
  const onboardingSkipBtn = document.getElementById('onboarding-skip-btn');

  if (onboardingOpenBtn) {
    onboardingOpenBtn.addEventListener('click', async () => {
      try {
        await window.luxshiftAPI?.requestAccessibility?.();
      } catch (_) { }
    });
  }
...
function getBlockTone(type) {
  switch (type) {
    case 'work': return { name: 'work', label: 'Work' };
    case 'unwind': return { name: 'unwind', label: 'Unwind' };
    case 'sleep': return { name: 'sleep', label: 'Sleep' };
    case 'wake': return { name: 'wake', label: 'Wake' };
    case 'break': return { name: 'break', label: 'Break' };
    case 'leisure': return { name: 'leisure', label: 'Leisure' };
    case 'meal': return { name: 'break', label: 'Meal' };
    case 'exercise': return { name: 'leisure', label: 'Exercise' };
    case 'study': return { name: 'work', label: 'Study' };
    case 'personal': return { name: 'general', label: 'Personal' };
    case 'commute': return { name: 'general', label: 'Commute' };
    case 'other': return { name: 'general', label: 'General' };
    default: return { name: 'general', label: 'General' };
  }
}
